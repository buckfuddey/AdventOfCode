...
https://adventofcode.com/2020/day/2
  ...
  Puzzle 1. 
    Find occurrance of string in string, e.g 4-10 s: ssskssphrlpscsxrfsr.
    Means: if s occurs in string between 4-10 times -> true
    
  Puzzle 2.
    Find position of string once, e.g 4-10 s: ssskssphrlpscsxrfsr.
    Means if s is in position 4 OR 10 but not both -> true
*/
...
foreach($asArray as $passwordData) {
    
    // split data into three pieces
    list($occurrance, $needle, $data) = explode(" ", $passwordData);
    
    // split min and max / positions 
    list($min, $max) = explode("-", $occurrance);

    // get letter by itself
    $needle = str_replace(":", "", $needle);

    // find all occurrances of letter in string for puzzle 1
    $count = preg_match_all('/['.$needle.']/', $data);

    // save if within boundaries of test one
    ...
    // save if within boundaries of test two
    if ( ($data[($min - 1)] == $needle) xor ($data[($max - 1)] == $needle) ) {

        $passed2++;
    }
}

var_dump("Puzzle 1: {$passed1}", "Puzzle 2: {$passed2}");
